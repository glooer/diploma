class Ui_Filter < Qt::MainWindow
	...
	def filter_apply
		...
		w = parse_where(where)
		limit = 1000
		results = @this.db.query("SELECT SQL_CALC_FOUND_ROWS lastName, firstName, patrName, birthDate, sex FROM Client WHERE #{w} LIMIT #{limit}")
		@this.db.query("SELECT FOUND_ROWS();").each{|v| p v}
